...
class UpdatePlaceTaskSet(SequentialTaskSet):
        ...
        try:
            ...
            try:
                ...
                if not json_data:
                    ...
                    self.place = random.choice(json.loads(json_data["body"]))
    ...
    def update_place(self):
        ...
        try:
            response = self.client.patch(f"{HOST_URL}/", headers=headers, json={"body": json.dumps(data)})
            response.raise_for_status()  # Raise an exception for non-2xx status codes
    ...
    def verify_update(self):
        ...
        try:
            response = self.client.get(f"{HOST_URL}/places?PlaceId={self.place_id}", headers=headers)
            response.raise_for_status()  # Raise an exception for non-2xx status codes
        except Exception as e:
...
if __name__ == "__main__":
    ...
    os.system("locust -f locust_rate_place_api.py")
